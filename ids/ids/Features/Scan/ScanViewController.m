...
@interface ScanViewController ()<DXAlertViewDelegate>
...
@property (nonatomic) Boolean needReturn; //回调2
@property (nonatomic, copy) QrcodeScan qrcodeScan; //回调2
@end
...
-(void)alertview:(NSString*)str
{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:str message:nil preferredStyle:UIAlertControllerStyleAlert];
    [self presentViewController:alert animated:NO completion:nil];
    [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(creatAlertview:) userInfo:alert repeats:NO];
}

- (void)creatAlertview:(NSTimer *)timer{
    UIAlertController *alert = [timer userInfo];
    [alert dismissViewControllerAnimated:YES completion:nil];
    alert = nil;
